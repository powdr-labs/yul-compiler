...
`findLabel`, and built-ins use the dialect's combined relational semantics.
...
def wimgFor (calls : ExternalCalls) (creates : ExternalCreates)
    (V : VEnv (evmWithExternal calls creates model.gas)) : List AVal :=
...
def namesFor (calls : ExternalCalls) (creates : ExternalCreates)
    (V : VEnv (evmWithExternal calls creates model.gas)) : List Ident :=
  V.map Prod.fst
...
def trimFor (calls : ExternalCalls) (creates : ExternalCreates) (depth : Nat)
    (V : VEnv (evmWithExternal calls creates model.gas)) : VEnv (evmWithExternal calls creates model.gas) :=
...
theorem asimE_op {prog : List Asm} {yst yst1 yst2 : EvmState}
    ...
    (hstep : builtinWithExternal extCalls extCreates model.gas yop args yst1 (.ok rets yst2)) :
...
theorem asimE_opHalt {prog : List Asm} {yst yst1 yst2 : EvmState}
    ...
    {args : List U256}
    (hargs : ASimE prog yst V off asm args yst1)
    (hstep : builtinWithExternal extCalls extCreates model.gas yop args yst1 (.halt yst2)) :
...
theorem builtin_iszero (v : U256) (st : EvmState) :
    builtinWithExternal extCalls extCreates model.gas .iszero [v] st
...
theorem asimE_switchCmp {prog : List Asm} {yst : EvmState} {V : VEnv yulD}
  ...
  have heq : builtinWithExternal extCalls extCreates model.gas .eq [w, cv] yst
