...
Extensions Mathlib does not provide (its `length_eq`/`imp` are used directly);
declared in `List.Forall₂` so dot-notation on `Forall₂` hypotheses finds them. -/
...
theorem set_positional : ∀ (V : VEnv D) (x : Ident) (v : D.Value),
  ...
  | cons p V ih =>
    ...
    · exact (if_pos h) ▸ List.Forall₂.cons ⟨h, Or.inr h⟩
        (List.Forall₂.refl (fun _ => ⟨rfl, Or.inl rfl⟩) V)
...
theorem mono {env : VMap} {V : VEnv yulD} {R R' : Regs} (h : EnvOK env V R)
  ...
  List.Forall₂.imp (fun _ _ hpq => ⟨hpq.1, hle _ _ hpq.2⟩) h
