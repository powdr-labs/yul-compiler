...
theorem isHaltingOp_halts {op : Op} (hop : isHaltingOp op = true)
    ...
    (hb : builtinWithExternal model.calls model.creates model.gas op args st r) :
    ∃ st', r = .halt st' := by
...
theorem builtin_eq (a b : U256) (st : EvmState) :
    builtinWithExternal model.calls model.creates model.gas .eq [a, b] st
...
theorem simS_op {P : Prog} {f : Func} {fn : FnState} {R : Regs}
    ...
    (hb : builtinWithExternal model.calls model.creates model.gas yop args st (.ok rets st'))
