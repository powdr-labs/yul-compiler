...
* `opStep` — the main per-built-in case analysis — and `pushStep`.
-/
...
/-- Executing an embedded `PUSH32 (conv u)` pushes `u`'s image and advances
the pc by 33. `StateMatch` is untouched (the instruction is pure). -/
theorem pushStep {code : ByteArray} {pre post : List UInt8} {u : U256}
    {yst : EvmState} {σ : List UInt256} {s : State}
    (hcode : code = mkCode (pre ++ (Instr.push (conv u)).bytes ++ post))
    (hf : FrameOK code s) (hm : StateMatch yst s)
    (hpc : s.pc = UInt256.ofNat pre.length)
    (hstk : s.stack = σ)
    (hgas : 40000 ≤ s.gasAvailable) :
    OkStep code s 40000 [u] yst pre.length 33 σ := by
  have hsz : code.size = pre.length + 33 + post.length := by
    subst hcode; simp [Instr.bytes]; omega
  ...
  refine ⟨_, EVM.Step.running hf.running hf.noPrecompile
    (StepRunning.pushN s ⟨32, by decide⟩ (conv u) 32 (by decide) hdec hgas'),
  ...
  · show conv u :: s.stack = [u].map conv ++ σ
    rw [hstk]
    rfl
