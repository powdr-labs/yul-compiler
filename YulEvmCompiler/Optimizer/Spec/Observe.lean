...
`Optimizer.LocalPass` demands `EquivBlock`: pointwise big-step equivalence — the
...
`LocalPass` (the `EquivBlock` tier) remains the production contract and the one
...
def ObsRefines (calls : ExternalCalls) (creates : ExternalCreates) (gasOracle : ExternalGas)
    (b₁ b₂ : Block Op) : Prop :=
  ∀ st0 V' st' o, Run (evmWithExternal calls creates gasOracle) b₁ st0 V' st' o →
    ∃ V'' st'', Run (evmWithExternal calls creates gasOracle) b₂ st0 V'' st'' o ∧
...
def ObsEquivBlock (calls : ExternalCalls) (creates : ExternalCreates) (gasOracle : ExternalGas)
    (b₁ b₂ : Block Op) : Prop :=
  ObsRefines calls creates gasOracle b₁ b₂ ∧ ObsRefines calls creates gasOracle b₂ b₁

namespace ObsRefines

theorem refl (b : Block Op) : ObsRefines calls creates gasOracle b b :=
  fun _ V' st' _ hrun => ⟨V', st', hrun, rfl⟩

theorem trans {b₁ b₂ b₃ : Block Op} (h₁ : ObsRefines calls creates gasOracle b₁ b₂)
    (h₂ : ObsRefines calls creates gasOracle b₂ b₃) : ObsRefines calls creates gasOracle b₁ b₃ := by
...
theorem refl (b : Block Op) : ObsEquivBlock calls creates gasOracle b b :=
  ⟨ObsRefines.refl b, ObsRefines.refl b⟩

theorem symm {b₁ b₂ : Block Op} (h : ObsEquivBlock calls creates gasOracle b₁ b₂) :
    ObsEquivBlock calls creates gasOracle b₂ b₁ := ⟨h.2, h.1⟩

theorem trans {b₁ b₂ b₃ : Block Op} (h₁ : ObsEquivBlock calls creates gasOracle b₁ b₂)
    (h₂ : ObsEquivBlock calls creates gasOracle b₂ b₃) : ObsEquivBlock calls creates gasOracle b₁ b₃ :=
...
theorem ofEquiv {b₁ b₂ : Block Op}
    (h : EquivBlock (evmWithExternal calls creates gasOracle) b₁ b₂) :
    ObsEquivBlock calls creates gasOracle b₁ b₂ :=
...
def ObsSound (run : Block Op → Block Op) : Prop :=
  ∀ (calls : ExternalCalls) (creates : ExternalCreates) (gasOracle : ExternalGas) (b : Block Op),
    ObsEquivBlock calls creates gasOracle b (run b)
...
def id : ObsPass where
  run := fun b => b
  sound := fun _ _ _ b => ObsEquivBlock.refl b
...
def comp (P Q : ObsPass) : ObsPass where
  ...
  sound := fun calls creates gasOracle b =>
    (Q.sound calls creates gasOracle b).trans
      (P.sound calls creates gasOracle (Q.run b))
...
def ofSound (run : Block Op → Block Op)
    (sound : ∀ (calls : ExternalCalls) (creates : ExternalCreates) (gasOracle : ExternalGas),
      Optimizer.Sound (evmWithExternal calls creates gasOracle) run) : ObsPass where
  run := run
  sound := fun calls creates gasOracle b =>
    ObsEquivBlock.ofEquiv (sound calls creates gasOracle b)
...
theorem optimize_then_compile_correct
    ...
    {yst0 : EvmState} {V' : VEnv (evmWithExternal model.calls model.creates model.gas)}
    {yst' : EvmState} {o : Outcome}
    (hrun : Run (evmWithExternal model.calls model.creates model.gas) prog yst0 V' yst' o) :
    ∃ V₂ yst₂,
      Run (evmWithExternal model.calls model.creates model.gas) (P.run prog) yst0 V₂ yst₂ o ∧
  ...
  obtain ⟨V₂, yst₂, hrun₂, hobs⟩ :=
    (P.sound model.calls model.creates model.gas prog).1 yst0 V' yst' o hrun
