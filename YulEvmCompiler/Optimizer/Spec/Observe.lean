...
def ObsRefines (calls : ExternalCalls) (creates : ExternalCreates)
    (gasOracle : ExternalGas) (b₁ b₂ : Block Op) : Prop :=
...
def ObsSound (run : Block Op → Block Op) : Prop :=
  ∀ (calls : ExternalCalls) (creates : ExternalCreates) (gasOracle : ExternalGas)
    (b : Block Op),
...
def comp (P Q : ObsPass) : ObsPass where
  ...
  sound := fun calls creates gasOracle b =>
    (Q.sound calls creates gasOracle b).trans (P.sound calls creates gasOracle (Q.run b))
...
def ofSound (run : Block Op → Block Op)
  ...
  run := run
  sound := fun calls creates gasOracle b => ObsEquivBlock.ofEquiv (sound calls creates gasOracle b)
