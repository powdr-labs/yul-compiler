...
/-!
# YulEvmCompiler.Optimizer.Core.Equiv

The optimization meta-theory foundations: **semantic equivalence** and its **congruence** properties
— the layer Yul→Yul optimization-pass correctness proofs stand on. Moved verbatim from
yul-semantics (`YulSemantics/Equiv.lean`, last at rev `d557aac`) when that repo was trimmed to pure
semantics; the `YulSemantics` namespace is kept so the many call sites (including the `Forall₂`
