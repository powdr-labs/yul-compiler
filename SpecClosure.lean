...
/-! ## CI gate 2 — regenerate the human-readable manifest.

Writing `SPEC.md` on every elaboration; CI runs `git diff --exit-code SPEC.md`
so the manifest cannot drift from the code. -/

open SpecClosure in
run_cmd do IO.FS.writeFile "SPEC.md" (report (← getEnv))
