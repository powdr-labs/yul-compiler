...
// bench/KeyValueList.sol

contract AGasTest {
  using KeyValueList for KeyValueList.List;

  function sortUpstreamVector() external pure returns (uint256[] memory) {
    // The exact eleven-cell vector from KeyValueList.t.sol's add test. It also
    // covers the sort test's collision ordering and uninitialized tail.
    ...
    list.add(6, 45e6, 10);
    list.add(7, type(uint32).max - 1, 10_000_000_000);
    list.add(10, 5e6, type(uint224).max - 1);
    list.sortByKey();
    return list._inner;
  }
}
