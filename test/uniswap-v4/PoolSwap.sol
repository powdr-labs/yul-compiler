...
//
// Uniswap v4-core Pool swap tier. The AGasTest wrapper is
// authored for this suite; the flattened library bodies below are verbatim
// v4-core sources from commit 46c6834698c48bc4a463a86d8420f4eb1d7f3b75
// (only import and individual pragma/SPDX lines are removed by flattening).
// Calls intentionally build on persistent state: add liquidity, swap, donate,
// remove liquidity, then read the resulting pool slot.
...
contract AGasTest {
    ...
    constructor() {
        pool.initialize(SQRT_PRICE_1_1, 3000);
    ...
    function swapExactInputNoTick()
        external
        returns (int128 amount0, int128 amount1, uint160 sqrtPriceX96, int24 tick)
    {
        (BalanceDelta delta,,, Pool.SwapResult memory result) = pool.swap(
            Pool.SwapParams({
        ...
        return (delta.amount0(), delta.amount1(), result.sqrtPriceX96, result.tick);
    }

    function donate() external returns (int128 amount0, int128 amount1) {
        BalanceDelta delta = pool.donate(1000, 2000);
        return (delta.amount0(), delta.amount1());
    }
    ...
    function slot0() external view returns (uint160 sqrtPriceX96, int24 tick, uint128 liquidity) {
        Slot0 value = pool.slot0;
        return (value.sqrtPriceX96(), value.tick(), pool.liquidity);
    }
}

// ----
// addLiquidityWide() -> -2955301087913717, -2955301087913717
// swapExactInputNoTick() -> -2000, 1993, 79228162514264193974492992661, -1
// donate() -> -1000, -2000
// removeLiquidity() -> 2990868880255737, 2990868880253924
// slot0() -> 79228162514264193974492992661, -1, 600000000000000000
