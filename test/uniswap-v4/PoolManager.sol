...
//
// Uniswap v4-core PoolManager integration tier. The AGasTest, PoolRouter, and
// TestToken harness contracts are authored for this suite; the flattened
// v4-core bodies below are verbatim sources from commit
// 46c6834698c48bc4a463a86d8420f4eb1d7f3b75 (only import and individual
// pragma/SPDX lines are removed by flattening). AGasTest inherits PoolManager
// so the harness can deliberately install its oversized top-level runtime in
// the local EVM while still exercising constructor CREATEs for the router and
// two tokens, cross-contract CALL callbacks, ERC20 settlement, and transient
// storage on every unlock.
...
// src/AGasPoolManager.sol

contract AGasTest is PoolManager {
    ...
    constructor() PoolManager(address(this)) {
        router = new PoolRouter(IPoolManager(address(this)));
        tokenA = new TestToken(address(router));
        tokenB = new TestToken(address(router));
    ...
    function initializeAndAddLiquidity() external returns (int128 amount0, int128 amount1) {
        this.initialize(key, SQRT_PRICE_1_1);
        BalanceDelta delta = router.run(PoolRouter.Action.AddInitialLiquidity);
        return (delta.amount0(), delta.amount1());
    }

    function swapExactInputNoTick() external returns (int128 amount0, int128 amount1) {
        BalanceDelta delta = router.run(PoolRouter.Action.SwapExactInputNoTick);
        return (delta.amount0(), delta.amount1());
    }

    function addLiquidityWide() external returns (int128 amount0, int128 amount1) {
        BalanceDelta delta = router.run(PoolRouter.Action.AddLiquidityWide);
        return (delta.amount0(), delta.amount1());
    }

    function donate() external returns (int128 amount0, int128 amount1) {
        BalanceDelta delta = router.run(PoolRouter.Action.Donate);
        return (delta.amount0(), delta.amount1());
    }

    function removeLiquidity() external returns (int128 amount0, int128 amount1) {
        BalanceDelta delta = router.run(PoolRouter.Action.RemoveLiquidity);
        return (delta.amount0(), delta.amount1());
    }
}
...
contract PoolRouter is IUnlockCallback {
    ...
    function run(Action action) external returns (BalanceDelta) {
    ...
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        ...
        if (action == Action.AddInitialLiquidity) {
        ...
        } else if (action == Action.SwapExactInputNoTick) {
            delta = manager.swap(
                key,
                SwapParams({
                    zeroForOne: true,
        ...
        } else if (action == Action.AddLiquidityWide) {
            (delta,) = manager.modifyLiquidity(
                key,
                ModifyLiquidityParams({
                    tickLower: -600,
            ...
            delta = manager.donate(key, 1000, 2000, "");
        } else {
            (delta,) = manager.modifyLiquidity(
                key,
                ModifyLiquidityParams({
    ...
    function _settle(Currency currency, int128 delta) private {
        if (delta < 0) {
            ...
            TestToken(Currency.unwrap(currency)).transfer(address(manager), uint256(uint128(-delta)));
            manager.settle();
        } else if (delta > 0) {
...
contract TestToken {
    ...
    constructor(address recipient) {
        balanceOf[recipient] = type(uint128).max;
    }
...
// ----
// initializeAndAddLiquidity() -> -5981737760509663, -5981737760509663
// addLiquidityWide() -> -2955301087913717, -2955301087913717
// swapExactInputNoTick() -> -2000, 1993
// donate() -> -1000, -2000
// removeLiquidity() -> 2990868880255737, 2990868880253924
