...
- `scripts/CheckSolidityGas.lean`: compiles Solidity contracts by feeding this compiler solc's *unoptimized* `--via-ir` Yul (`--ir`, front-end only) and gas-compares against solc's *optimized* runtime (`--bin-runtime --optimize --via-ir`). Both compilers' optimized creation bytecode is deployed and the contract's calls are replayed on each: semanticTests fixtures' own `// ----` call specs (parsed by `YulEvmCompilerTests/SolTest.lean` into calldata + value, arguments already in flattened ABI form) for real calls, or one synthetic call per selector for gasTests. Runs the curated `libsolidity/gasTests` strictly (all must compile; `test/solidity-gas-baseline.txt`), the large `libsolidity/semanticTests` corpus in `--lenient` mode, sharded (`test/solidity-semantic-gas-baseline.txt`), and the in-repo Uniswap v4-core fixtures (`test/uniswap-v4/`, see `test/README.md`) strictly against a known-compile-failures list (`--known=test/uniswap-v4-known-compile-failures.txt`, `test/uniswap-v4-gas-baseline.txt`). Built as the native `checkSolidityGas` executable (declared in `lakefile.toml`), not run through `lean --run`, because compiling large real-world contracts recurses past the interpreter stack; CI and `update-gas.sh` `lake build` it and raise the OS stack (`ulimit -s`). Relies on three dialect accommodations in the front-end: `YulParser.Validate.sourceNumbersWF` skips identifiers (so digits inside solc's hashed names are not mis-lexed as numbers), and `YulParser.compileSource` desugars the `memoryguard` hint to its argument and drops `linkersymbol` let-bindings whose variable is provably never referenced (the dead placeholder solc's unoptimized IR emits for qualified internal library calls; a program that uses the value is still rejected) before compilation.
