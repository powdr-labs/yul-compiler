...
- `scripts/CheckSolidityGas.lean`: compiles Solidity's `libsolidity/gasTests` contracts via solc's `--via-ir` optimized-Yul pipeline and gas-compares the optimized runtime against solc's (`test/solidity-gas-baseline.txt`). Relies on two dialect accommodations in the front-end: `YulParser.Validate.sourceNumbersWF` skips identifiers (so digits inside solc's hashed names are not mis-lexed as numbers), and `YulParser.compileSource` desugars the `memoryguard` hint to its argument before compilation.
