...
- `StateMatch` currently relates memory contents and the active-memory high-water mark, calldata, executing and external code, returndata, environment/header readers, account balances, and the executing account's persistent/transient storage. Extend it before proving operations that observe or mutate other state such as logs or further account data.
